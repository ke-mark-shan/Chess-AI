...
public class Pawn extends ChessPiece {
	...
	private boolean canDiagonalAttack(int col, int row){
		...
		return (this.getModel().inBoard(col) &&
				null != super.getModel().getBoard().getPiece(new Position (col, row)) &&
				super.getPlayerColour() != super.getModel().getBoard().getPiece(new Position(col, row)).getPlayerColour() &&
	...
	public ArrayList<Position> getPossibleMoves(){
		...
		if (this.getModel().inBoard(row + direction)){
			// Movement forward
			if (null == super.getModel().getBoard().getPiece(new Position(col, row + direction))){
				// Can move one cell forward
				if (!this.tryMoveCheck(new Position(col, row + direction))){
					moves.add(new Position(col, row + direction));
				}
				if (!this.madeMove &&
					null == super.getModel().getBoard().getPiece(new Position(col, row + 2 * direction)) &&
